...
export default async function BlockDetailPage({ params }: PageProps) {
  ...
  const PreviewComponent = dynamic(
    () => import(`../../../../registry/blocks/${slug}.tsx`).catch(() => {
      return () => <div className="p-4 text-center text-zinc-500 border border-dashed rounded-md m-4">Block preview not available. Ensure the file exists in the registry.</div>
    }),
    { 
